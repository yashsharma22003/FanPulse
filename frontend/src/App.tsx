...
import { ArrowDownRight, ArrowUpRight, BarChart3, ChevronRight, CircleHelp, Clock3, Copy, Flame, Gauge, LoaderCircle, LockKeyhole, Menu, RefreshCw, ShieldCheck, Swords, Trophy, Wallet, X, Zap } from 'lucide-react';
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const active = NAV.find((item) => item.href === location)?.label ?? (location.startsWith('/market') ? 'Arena' : location.startsWith('/profile') ? 'Profile' : location.startsWith('/challenge') ? 'Clashes' : '');
...
function MarketCard({ market, index = 0 }: { market: any; index?: number }) {
  ...
  return <Link href={`/market/${market.marketId}`} className="group rise block rounded-[22px] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[var(--shadow-sm)] transition-transform hover:-translate-y-1 hover:shadow-[var(--shadow-lg)]" style={{ animationDelay: `${index * 70}ms` }} data-testid={`card-market-${market.marketId}`}>
    <div className="mb-7 flex items-start justify-between"><div><div className="flex items-center gap-2"><span className="font-display text-lg font-bold">{market.asset}</span><StatusPill tone={market.tradable ? 'lime' : 'muted'}>{market.tradable ? 'Live' : 'Locked'}</StatusPill></div><p className="mt-1 font-mono-ui text-[10px] uppercase tracking-[.13em] text-[hsl(var(--muted-foreground))]">{market.intervalSec / 60 >= 60 ? `${market.intervalSec / 3600} hour` : `${market.intervalSec / 60} min`} call · {market.venueId ?? 'Somnia venue'}</p></div><ChevronRight size={18} className="text-[hsl(var(--muted-foreground))] transition-transform group-hover:translate-x-1" /></div>
...
function Home() {
  ...
  return <AppShell><div className="arena-grid px-5 py-8 sm:px-8 sm:py-10 lg:px-10"><div className="mx-auto max-w-[1280px]"><PageIntro eyebrow="Live from Somnia Shannon" title={<>Make the call.<br /><span className="text-[hsl(var(--accent))]">Own the moment.</span></> }><Link href="/leaderboard" className="flex items-center gap-2 text-xs font-bold text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]" data-testid="link-see-rankings">See who is on fire <ChevronRight size={15} /></Link></PageIntro>{query.isError && <ConnectionState error onRetry={() => query.refetch()} />}<section className="mb-12 grid gap-5 lg:grid-cols-[1.55fr_1fr]"><div className="relative overflow-hidden rounded-[28px] bg-[hsl(var(--primary))] p-6 text-[hsl(var(--primary-foreground))] shadow-[var(--shadow-lg)] sm:p-9"><div className="absolute -right-14 -top-20 h-64 w-64 rounded-full border-[38px] border-[hsl(var(--secondary)/.25)]" /><div className="absolute bottom-0 right-12 h-32 w-32 rotate-45 bg-[hsl(var(--accent)/.9)] opacity-90" /><div className="relative z-10 max-w-xl"><div className="mb-12 flex items-center justify-between"><StatusPill tone="lime"><span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--primary))]" />Featured opportunity</StatusPill><span className="font-mono-ui text-[10px] uppercase tracking-[.15em] opacity-55">01 / 04</span></div><p className="mb-2 font-mono-ui text-[10px] uppercase tracking-[.17em] opacity-55">The crowd is leaning</p><h2 className="font-display text-4xl font-extrabold tracking-[-.06em] sm:text-6xl">{featured.asset}</h2><p className="mt-3 max-w-sm text-sm leading-6 opacity-70">Will the reference price close higher than it opened?</p><div className="mt-10 flex flex-wrap items-end gap-8"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.13em] opacity-55">AI says Up</p><p className="font-display text-5xl font-bold text-[hsl(var(--secondary))]">{featured.aiPercent}%</p></div><div className="h-12 w-px bg-white/20" /><div><p className="font-mono-ui text-[10px] uppercase tracking-[.13em] opacity-55">Time to lock</p><p className="font-mono-ui text-xl font-bold">{formatTime(featured.secondsLeft)}</p></div></div><Link href={`/market/${featured.marketId}`} className="mt-9 inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--secondary))] px-5 py-3.5 text-sm font-bold text-[hsl(var(--primary))] transition-transform hover:-translate-y-0.5" data-testid="link-featured-market">Enter the arena <ArrowUpRight size={16} /></Link></div></div><div className="rounded-[28px] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-[var(--shadow-sm)] sm:p-8"><div className="mb-8 flex items-center justify-between"><p className="font-mono-ui text-[10px] font-bold uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Arena pulse</p><BarChart3 size={18} className="text-[hsl(var(--accent))]" /></div><div className="space-y-6"><div><div className="mb-2 flex justify-between text-xs"><span>Calls made today</span><strong className="font-mono-ui">1,284</strong></div><div className="h-2 rounded-full bg-[hsl(var(--muted))]"><div className="h-full w-[72%] rounded-full bg-[hsl(var(--primary))]" /></div></div><div><div className="mb-2 flex justify-between text-xs"><span>Energy in play</span><strong className="font-mono-ui">48,920</strong></div><div className="h-2 rounded-full bg-[hsl(var(--muted))]"><div className="h-full w-[58%] rounded-full bg-[hsl(var(--accent))]" /></div></div><div className="rounded-2xl bg-[hsl(var(--muted))] p-4"><p className="font-mono-ui text-[10px] uppercase tracking-[.13em] text-[hsl(var(--muted-foreground))]">Last settlement</p><div className="mt-3 flex items-center justify-between"><span className="font-display text-2xl font-bold">SOMI / USD</span><StatusPill tone="coral">Up won</StatusPill></div><p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">31 fans read the tape right</p></div></div></div></section><div className="mb-5 flex items-end justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Markets in play</p><h2 className="mt-1 font-display text-2xl font-bold tracking-[-.04em]">Pick your pressure point</h2></div><span className="font-mono-ui text-[10px] text-[hsl(var(--muted-foreground))]">{markets.length} LIVE WINDOWS</span></div>{query.isLoading ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><SkeletonCard /><SkeletonCard /><SkeletonCard /><SkeletonCard /></div> : markets.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{markets.map((market: any, index: number) => <MarketCard key={market.marketId} market={market} index={index} />)}</div> : <div className="rounded-2xl border border-dashed border-[hsl(var(--border))] p-12 text-center" data-testid="empty-markets"><p className="font-display text-xl font-bold">No markets are live right now.</p><p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">The arena is between rounds. Check back soon.</p></div>}</div></div></AppShell>;
}
...
function WalletPredictionPanel({ detail }: { detail: any }) {
  ...
  return <div className="rounded-[24px] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[var(--shadow-sm)] sm:p-6">
    <div className="mb-5 flex items-center justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.15em] text-[hsl(var(--muted-foreground))]">Your call</p><h3 className="mt-1 font-display text-xl font-bold">Choose a side</h3></div><StatusPill tone="muted"><LockKeyhole size={11} />Wallet sign required</StatusPill></div>
...
function ChallengesPage() {
  ...
  return <AppShell><div className="px-5 py-8 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1100px]"><PageIntro eyebrow="Public prediction feed" title={<>Find your<br /><span className="text-[hsl(var(--accent))]">opponent.</span></>}><div className="rounded-xl bg-[hsl(var(--secondary)/.45)] px-4 py-3 text-right"><p className="font-mono-ui text-[9px] uppercase tracking-[.12em]">Open calls</p><p className="font-display text-2xl font-bold">{open.length}</p></div></PageIntro>{query.isError && <ConnectionState error onRetry={() => query.refetch()} />}<div className="mb-7 flex items-center justify-between border-b border-[hsl(var(--border))] pb-4"><p className="text-sm text-[hsl(var(--muted-foreground))]">Opposite sides only. Better judgment makes better rivals.</p><button onClick={() => query.refetch()} className="flex items-center gap-2 font-mono-ui text-[10px] uppercase tracking-[.1em] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]" data-testid="button-refresh-challenges"><RefreshCw size={13} /> Refresh</button></div>{query.isLoading ? <div className="grid gap-4 md:grid-cols-2"><SkeletonCard /><SkeletonCard /></div> : open.length ? <div className="grid gap-4 md:grid-cols-2">{open.map((item: any) => <ChallengeCard key={item.id} item={item} onChallenge={setSelected} />)}</div> : <div className="rounded-[22px] border border-dashed border-[hsl(var(--border))] p-12 text-center" data-testid="empty-challenges"><Swords className="mx-auto text-[hsl(var(--accent))]" /><p className="mt-4 font-display text-xl font-bold">No open calls yet.</p><p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">Place a prediction to become the first call on the feed.</p></div>}</div></div>{selected && <WalletChallengeModal item={selected} onClose={() => setSelected(null)} />}</AppShell>;
}
